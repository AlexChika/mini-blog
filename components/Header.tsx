...
const Header = () => {
  return (
    <header className="flex items-center justify-between space-x-2 font-bold px-3 sm:px-10 py-2 drop-shadow-sm">
      <div className="flex items-center space-x-2">
        ...
        <Link href="/">
          <h1 className="italic py-2 px-1 flex">
            <span className="text-xs sm:text-sm">Mini Blog</span>
              ...
              {[5, 7, 9].map((s, ind) => {
                return (
                  <span
                    ...
                    className={`block group-hover:bg-white transition-colors duration-200 ease-out bg-[#ff8a75] rounded-full mr-1`}
                    key={ind}
                  ></span>
                );
              })}
      ...
      <div>
        <Link
          className="px-2 sm:px-5 py-1 text-sm md:text-base bg-gray-900 text-primary flex items-center rounded-full text-center"
